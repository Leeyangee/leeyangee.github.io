...
经过一些时间的排查后，终于发现一个比较可疑的Web应用，虽然该Web应用在互联网中的资产十分少，但却具有惊人的通用性  
...
在粗略地通看了一遍后，发现并未包含主要的敏感函数和漏洞利用链，就算是"subprocess.Popen"、"os.Popen"等函数，都采用了较为严格的列表式和严格的过滤，极难构造命令执行
