...
经过资产侦察发现，该应用为通杀系统，却在互联网上仅有约二十条IP，这不得不更增添了我对它的好奇  
...
经过不断搜集，笔者终于发现了某个较为远古的开源仓库的commits Delete中遗留下来的该Web应用的身影.  
...
该Web应用是一个服务于该厂各业务线上的分布式测试框架，并且一直是仅限于内部使用. 后面大概在该框架发行大概两年 ~ 四年后推出了该框架的开源分支，不过现如今该开源分支已经完全停止维护.  不过从现有掌握到的线索来看，非开源版本一直在持续维护并被部署在各业务线中
...
后面在看了看其他的Api Handler，没什么包含大危害的点. 本来差点就想部署分布式去测了，但在临走之前鬼使神差地看了眼加载静态页面的如下Handler，没曾想直接发现了RCE  
