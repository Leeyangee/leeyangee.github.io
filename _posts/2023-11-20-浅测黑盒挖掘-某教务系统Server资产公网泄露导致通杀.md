...
到这里可能一些读者对于Soap协议有点一头雾水，Soap协议的解释和说明如下所示  
...
这样其实我们就可以把Soap协议简单理解为 按XML数据格式基于http协议规范传输的通信协议  
类似于现在常用的按json数据格式传输的后端  

现在省去手动调试的麻烦，直接用SoapUI来调试该SoapService接口
