...
本篇文章为笔者自己的教程分享目录，除开特别说明的 Payload 、`//Payload`代码标注之外，带有注释的程序源代码分析片段如`//源码分析`、笔者个人思考内容未经许可禁止转载
...
为啥 Filter 里需要调用 filterChain.doFilter 呢？我们分析一下 FilterChain，直接进入 FilterChain 的实现类即 catalina.core.ApplicationFilterChain 查看源码，发现如下所示: doFilter 方法先检查其全局安全性后，最终跳到 this.internalDoFilter 方法中
...
而 internalDoFilter 方法的详细分析如下所示，我们发现了在每次 doFilter 被用户/系统定义的 Filter 触发时，其本质上就是逐次取出 filters 中的后一个 ApplicationFilterConfig 进行轮转依次调用 Filter，当轮转结束后直接转到 this.servlet.service  
...
接下来就简单许多了，笔者相信各位读者都注意到了 createFilterChain 那两层循环中的某行代码，用于获取当前 filter 的 filterConfig
...
我们跟进 StandardContext.findFilterConfig，发现 findFilterConfig 就是单单返回当前 filterName 对应的 filterConfig
...
跟进到抽象类 org.apache.catalina.core.ContainerBase 后，其先调用 addChild 而后跟进调用 addChildInternal
...
那各位可能疑问又来了: 我知道这个 this.children 数组是存 StandardWrapper 的，那他又是在何处被调用的呢？  
我们继续跟进源码，观察到有个 findChildren 函数返回 Container[] 数组类型的 children
...
public abstract class ContainerBase extends LifecycleMBeanBase implements Container {
    ...
    public Container[] findChildren() {
        synchronized(this.children) {
            return (Container[])this.children.values().toArray(new Container[0]);
        }
    }
}
```

在其继承类 StandardContext 中跟进这个函数，即可观察到 startInternal 生命周期中存在这么几行关键代码
...
public class StandardContext extends ContainerBase implements Context, NotificationEmitter {
    protected synchronized void startInternal() throws LifecycleException {
        try {
            if (ok) {
                ...[省略几千行代码]...
                Container[] var35 = this.findChildren();
                int var8 = var35.length;
                //遍历 children 数组
                for(int var9 = 0; var9 < var8; ++var9) {
                    Container child = var35[var9];
                    //判断是否 Available
...
```
因此可得知，这个 Wrapper 入口点 start 是 StandardConext 生命周期中被调用的  
至此，对于 addChild 的分析路径暂时告一段落，在后续笔者再更新其他内容

回归正题，在 StandardContext.addChild(Wrapper) 后，直接如下所示调用 StandardContext.addServletMappingDecoded 进行Mapping注册  
对于 StandardContext.addServletMappingDecoded，较为简单笔者不再过多赘述
