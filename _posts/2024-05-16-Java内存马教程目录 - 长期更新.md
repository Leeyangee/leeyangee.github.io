...
| [&nbsp;&nbsp;&nbsp;1.1.1 Filter型内存马及部分源码分析](#112-filter型内存马) |
| [&nbsp;&nbsp;&nbsp;1.1.2 Servlet型内存马简述](#111-servlet型内存马) |
...
内存马是什么？内存马即是驻留在内存中的木马，可以通过 API调用触发他
...
StandardContext 中关于 Filter 的重要参数:  

1. FilterDefs: 存放Filter配置  
2. FilterMaps: 将 FilterConfig 映射到具体请求路径上  
3. FilterConfig: Filter的具体配置，在正常业务下注册时无需配置

其在 org.apache.catalina.core.StandardContext 中的数据结构如下所示  
```java
//StandardContext
...
1. FilterChain: 由多个 FilterConfig 组成的链式结构  

这里，直接点 FilterChain 的实现类源码查看，即 org.apache.catalina.core.ApplicationFilterChain，发现如下所示: 其 doFilter 方法先检查其全局安全性后，最终跳到 this.internalDoFilter 方法中
```java
...
```
而 internalDoFilter 方法的详细分析如下所示，我们发现了在每次 doFilter 触发时，其本质上就是取出 filters 中的 ApplicationFilterConfig[] 然后进行逐个轮转，代码分析如下注释所示
```java
public final class ApplicationFilterChain implements FilterChain {
