---
title: Java内存马/中间件源码剖析 - 长文/长期更新
published: true
---

内存马（Memory Shell）是一种利用各类中间件、服务器漏洞，将恶意代码注入中间件、服务器进程的内存中的攻击技术.  
其特点是无文件木马，无文件落地，通常会存在进程，内存或者java虚拟机中，更加隐蔽，难以排查，难以删除. 

本篇文章为笔者自己的教程分享目录，除开特别说明的 Payload 代码标注之外，带有注释的程序源代码分析片段、笔者个人思考内容未经许可禁止转载
...
1. FilterChain: 由多个包含 FilterConfig 组成的链式结构，其参数位置如下所示  
    ...
    public class TestFilter implements Filter{
        @Override
        //doFilter 中包含的 FilterChain 及其后续 Filter 调用链
...
在这里我们分析一下 FilterChain，直接进入 FilterChain 的实现类即 catalina.core.ApplicationFilterChain 查看源码，发现如下所示: doFilter 方法先检查其全局安全性后，最终跳到 this.internalDoFilter 方法中
...
而 internalDoFilter 方法的详细分析如下所示，我们发现了在每次 doFilter 被用户/系统定义的 Filter 触发时，其本质上就是逐次取出 filters 中的后一个 ApplicationFilterConfig 进行轮转，当轮转结束后直接转到 this.servlet.service  
在这里 pos 变量即为 FilterConfig 的遍历下标，n 变量即为总的 FilterConfig 个数
代码分析如下注释所示
...
我们知道，Servlet的注册流程如下:  
首先调用 Tomcat.addServlet(Servlet)，观察 addServlet 函数源代码发现 addServlet 函数就是先 new ExistingStandardWrapper(Servlet)，该对象 后位于 Tomcat 主类中继承自 StandardWrapper，然后再 Wrapper.setName(ServletName)，而后再顺便 context.addChild(Wrapper)，最后返回一个 Wrapper 对象.  
...
那么这个 context.addChild(Wrapper) 到底干了什么呢？我们直接点开 StandardContext 源码查看，发现该实现主要判断了下是否为 jsp Servlet 而已，若为 jsp Servlet 即将其 addServletMappingDecoded 中.  
我们继续跟进 super.addChild
...
对于 StandardContext.addServletMappingDecoded，其路径较为简单笔者不再过多赘述
