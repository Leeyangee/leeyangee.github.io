...
继续跟进 ApplicationFilterFactory.createFilterChain 被调用的地方 org.apache.catalina.core.StandardWrapperValve，发现 servlet 实际来源于 wrapper.allocate()
...
wrapper.allocate() 中到底有什么笔者不细写了，关键代码如下所示
...
我们发现，这个 wrapper.allocate() 返回的是 instance，instance才是事情的关键

那到底这个 instance 从何而来呢？我们细细探究发现一行代码: this.instance = this.loadServlet()，instance 被初始化于 Tomcat 类中类 ExistingStandardWrapper 中的方法 ExistingStandardWrapper.loadServlet ，如下所示
...
兜兜转转最终居然又绕回来了，这也标志 Servlet 型内存马源码分析完结，根据上述路径，可以完整构造出 Payload
