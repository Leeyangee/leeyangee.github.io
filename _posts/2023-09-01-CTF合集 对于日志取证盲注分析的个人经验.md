...
然后给出了一大堆黑客活动产生的日志记录，经浓缩后数据大致如下所示
```
...
众所周知的，盲注的主要过程是：逐语句地一个个试字符串中第n个字符的ascii码值(0 <= n < 字符串长度)，对每次盲注response分析后缩小该第n个字符的可能ascii范围，反复 试探-分析-缩小范围 这个过程，最终确定第n个字符的编码值. 因此盲注一般花费的时间非常长，经常需要跑很久
