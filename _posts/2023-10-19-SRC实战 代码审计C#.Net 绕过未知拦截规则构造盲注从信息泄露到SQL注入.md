---
title: SRC实战 代码审计C#.Net 绕过未知拦截规则构造盲注从信息泄露到SQL注入
published: true
---
# [](#header-1)0、前言
...
namespace SF_WEB.Controllers
  class AccountControllers
    function LogOn()
    function LogOn(LogOnModel, string)
    function LogOnSSO(string)
```
源代码太长了，部分代码如下所示
...
分析了一下原因，发现极有可能是执行SQL命令的函数出现了问题  
```c#
this.dao.GetDataTable(text);
```
其隶属于  
```c#
namespace APP.DAL
	interface IDataAccessObject
		function GetDataTable(string queryString);
```
原因我暂时还没有分析出来，问了一下csdn知乎上的老哥，说的该函数只需要参数是SQL命令即可执行. 与该项目测试结果相悖，我猜测可能有未知waf挡了

# [](#header-1)5、结束

后面本来还想继续测试的，因为在审计时发现了几个疑似的文件上传点和逻辑漏洞，但后面在测试ChangePassword接口时在Update语句中执行了OR语句(搞不好整个表的用户信息都给他改了，人都麻了)，现在正在排查风险中. 因此不敢再继续测下去了
