...
我起初认为这个问题很好解决，有很多url修改域名的方案. 但我搜索了一下，发现带路径的url根本无法修改域名. 遂放弃修改域名

然后我直接在该网站的搜索栏、错误界面中搜索看看返回的数据是否可控，发现都失败了. 该网站正常业务处理请求，所返回的数据中均不携带任何原先请求的信息(我猜想网站建设者在建设时也考虑到了这一点)  
