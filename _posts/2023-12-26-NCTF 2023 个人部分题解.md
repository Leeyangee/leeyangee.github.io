...
在这里，使用Invoke-Image技术可以解png图片，这里的exp代码以后再展示  
解码后是一堆混淆powershell代码，怕一些层中间藏有信息，因此我们层层剥除来解混淆  
...
(NEw-ObjeCt  sySTeM.iO.sTReamreadEr(
    (NEw-ObjeCt  Io.cOMPrEssIoN.DEflATeSTREaM(
      [sYsTEM.iO.MemoRYsTReaM][cOnVert]::frOMbAsE64StRinG( '......' ) ,[Io.cOMpReSsiON.cOMPreSsIonMoDe]::dEcOmprESs
    )) , [tEXT.EncoDING]::aScII
)).reADTOeNd()
```
...
```
这层我们逐个剥除慢慢试. 不难发现，首先在最外层加个()后，再去掉```| & ( $EnV:COmspEc[4,15,25]-jOIN'')```，最前面再加个echo就是进入第三层的答案  
得到进入第三层的代码是:  
```powershell
echo ((
  '密文'.sPLIt('<r_l:{&Z' )
...
```
这层虽然看起来很抽象，但是仔细看代码还是不难懂. 把前面```.(([STRINg]$VeRbOSEPrefEReNcE)[1,3]+'X'-jOIN'')```这个执行的部分删掉，保留后面解密的代码就行了  
得到进入第四层的代码是:  
```powershell
