---
title: 代码审计.Net 绕过未知拦截规则从信息泄露到SQL注入
published: true
---
# [](#header-1)0、前言

首先，这是我第一次深入的对Web应用的.Net代码审计. 之前虽审计过由py、java组织的项目以及基于Unity技术栈的C#项目，但从未对Web应用的.Net深入的代码审计过，因此将其记录下来.  
虽然之前也得到过一些资产的DLL源码，但鉴于基本上是在已经GetShell的情况下获取的，且是单站点应用. 因此并无深入审计拿通杀的必要  
...
```
原因我暂时还没有分析出来，问了一下CSDN或知乎上对于该方面有经验的各位，大多数都指明该函数只需要参数是SQL命令即可执行. 与该项目测试结果相悖，我猜测可能有未知waf挡了
