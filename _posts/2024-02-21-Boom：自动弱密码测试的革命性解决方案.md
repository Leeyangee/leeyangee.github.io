...
其对于网站登录form表单的识别率、编译型vue前端、传统认证的识别率都远超我的期望，甚至于相比于传统模糊弱密码测试工具已经到了遥遥领先的水平  
...
笔者在这里选取了几个网站进行测试，均收获了较好的结果. 这里拿我一个朋友的网站登陆后台来举例说明  
其技术栈为前端: vue 组件 ant-design，后端: go 基础组件为 gin  

后台登陆的url为: http://xxx/login
![avatar](/image/2024-02-21-3.png)

登录请求url为: http://xxx/api/email_login
![avatar](/image/2024-02-21-4.png)

仔细查看该后台页面前端源代码发现，其登陆发请求逻辑根本不是传统form表单，而是由vue编译后的，由html登录点击事件跳转到js事件响应器再发请求的 vue请求  
要想准确识别到该网站的请求逻辑，其难度不言而喻  
