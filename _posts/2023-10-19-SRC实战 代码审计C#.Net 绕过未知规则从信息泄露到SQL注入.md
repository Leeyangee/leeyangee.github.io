...
```
当此时password的值正好等于version()第一个字符拼接6次后，该select语句返回userid，否则不返回东西. 可以根据此规则进行盲注
为什么要重复六次substring(version(),1,1)呢？因为password长度必大于等于6，substring(version(),1,1)必须拼接6次  

现在我们根据规律，令password等于'000000'、'111111'、'222222'....以此遍历来慢慢找到version()的第一个字符  
