...
既然WebValidateUser中username、password都是我们输进去的值，并且将username、password格式化进SQL语句并进行查询，那这样就方便了  
...
我尝试了几个简单的注入payload，发现请求直接没有返回值了  
我以为是简单的payload出了点问题. 于是我又输入了几个注入payload在password和username，又没反应. 我已经感觉到不对劲了  

但是在这里，无论我尝试了多少种payload，都绕不过去  
于是我转头放弃，去分析其他的api接口  

# [](#header-1)3、深入分析

在大批量fuzz后，我终于发现了一个能用的接口观测到的疑似规律  
在ChangePassword(ChangePasswordModel)中  
```
SF_WEB.Controllers
  AccountControllers
...
```
在这一行代码中，studentId是系统根据session查出来的，oldPassword是用户自定义的  
当此时payload为 ' or password!='1 时，能注入成功  

总结了一下这条规律:  
1、注入的payload的字段必须是password，例如 ' or password != '' and password = '  
2、以上规则只在/Account/ChangePassword接口触发  
3、payload若包含关系运算符，则左侧必须为password字段，例如 ' or password = if(password = '1','1','0') and password = '  
4、password字段的长度不可能低于6  

在这里，看似很难利用，但我们可以直接构造盲注语句  
```sql
' and password = concat(substring(version(),1,1),substring(version(),1,1),substring(version(),1,1),substring(version(),1,1),substring(version(),1,1),substring(version(),1,1)) + '
...
```
为什么要重复六次substring(version(),1,1)呢？因为password长度必大于等于6，因此我们必须拼接6次  
我们可以遍历令password等于'000000'、'111111'、'222222'来慢慢找到version()的第一个字符  

再经过几次遍历后，终于找到了version()的第一个字符为5  

多次遍历后，终于找出了version()的值为: 5.6.49-log
