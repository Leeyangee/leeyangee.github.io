...
当此时oldPassword = ' or password!='1 时，该SQL命令返回正常值，说明此处能注入绕过  
...
在这里，看似很难利用，但password是可以通过正常业务更改的，因此可以直接在这里构造盲注语句  
```
...
```
当此时password的值正好等于version()第一个字符拼接6次后，该select语句返回studentId，否则不返回任何值. 可以根据此规则进行盲注  
为什么要重复六次substring(version(),1,1)呢？因为password长度必大于等于6，substring(version(),1,1)必须拼接6次  

现在根据规律，写个脚本令password等于'000000'、'111111'、'222222'....以此遍历来慢慢找到version()的第一个字符  
